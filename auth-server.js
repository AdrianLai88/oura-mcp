#!/usr/bin/env node
//
// Handles signing in, and lets nothing through to the Oura MCP without it.
...
if (!ISSUER) {
  console.error('ISSUER is required, e.g. https://oura-mcp.example.com');
  process.exit(1);
}
...
const CONFIG_DIR = process.env.CONFIG_DIR || path.join(os.homedir(), '.config', 'oura-mcp');
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  <form method="POST" action="/authorize">
      ...
      <h1>Oura</h1>
      <p>Sign in to give this app access to your Oura data.</p>
...
app.post('/token', cors, (req, res) => {
  ...
  if (body.grant_type === 'authorization_code') {
    ...
    return res.json(issueTokens(clientId, row.resource || RESOURCE, row.scope || 'oura'));
  }
  ...
  if (body.grant_type === 'refresh_token') {
    ...
    return res.json(issueTokens(clientId, row.audience || RESOURCE, row.scope || 'oura'));
  }
...
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Oura MCP</title>
<meta name="description" content="MCP server for Oura ring data.">
<link rel="icon" href="/favicon.ico" sizes="any">
<link rel="icon" href="/icon.png" type="image/png">
<link rel="apple-touch-icon" href="/icon.png">
<style>
